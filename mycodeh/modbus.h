...
// Modbus寄存器地址定义
#define MODBUS_REG_TEMPERATURE_ADDR  0x00c8  // 温度值寄存器地址（第3个位置）
#define MODBUS_REG_PRESSURE_ADDR     0x00C9  // 压力值寄存器地址（第2个位置）
#define MODBUS_REG_STATUS_ADDR       0x00ca  // 状态寄存器地址（第4个位置）
...
typedef struct {
    ...
    // 温度传感器数据
    double temperature_value;   // 温度值 (°C)
    uint32_t temperature_timestamp; // 温度值时间戳
    ...
    // 传感器状态标志
//    uint8_t i2c_status;         // I2C通信状态
//    uint8_t uart_status;        // UART通信状态
//    uint8_t ble_status;         // BLE连接状态
} GlobalSensorData_t;
